	...
	var setAPIOrigin = function setAPIOrigin(location, config) {
	  ...
	  if (search.environment === 'development' || location.hostname === 'localhost' && search.environment !== 'production') {
	    if (!(config.apiOrigin = config.apiOrigins.development)) {
	      ...
	      config.apiOrigin = 'https://aqueous-atoll-85096.herokuapp.com';
	    }
	  } else {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  api.signUp(data).then(ui.signInSuccess).catch(ui.signInSuccess);
	};
	...
	var onNewGame = function onNewGame(event) {
	  ...
	  api.newGame(data).then(ui.newGameSuccess).catch(ui.newGameFail);
	};

	// create function to check if winning combo is in the array
	var checkForMatch = function checkForMatch(valuesArray, tic, tac, toe) {
	  // match arguments with valuesArray to check for winner
	  if (valuesArray.indexOf(tic) !== -1 && valuesArray.indexOf(tac) !== -1 && valuesArray.indexOf(toe) !== -1) {
	    gameOver = true;
	  }
	};
	// created funtion that controls turn taking
	var turnSwitch = function turnSwitch() {
	  var idOfPick = event.target.id;
	...
	var onUpdateGame = function onUpdateGame(event) {
	  ...
	  api.updateGame(data).then(ui.updateGameSuccess).catch(ui.failure);
	};

	var onGameIndex = function onGameIndex(event) {
	  event.preventDefault();
	  api.gameIndex().then(ui.gameIndexSuccess).catch(ui.gamefailure);
	};
